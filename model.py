...
class DiscreteGraphLearning(nn.Module):
    ...
    def __init__(self, dataset_name, k, input_seq_len=2016, output_seq_len=12, data_number=0):
        super().__init__()

        self.k = k          # the "k" of knn graph
        self.num_nodes = {"METR-LA": 207, "PEMSD7M": 228, "PEMS-BAY": 325, 'HKTSM':608}[dataset_name]
        self.train_length = {"METR-LA": 23990, "PEMSD7M": 8870, "PEMS-BAY": 36482, 'HKTSM':21152}[dataset_name]
        self.node_feats = torch.from_numpy(load_pkl("../data/" + dataset_name + "/data_in{0}_out{1}.pkl".format(input_seq_len, output_seq_len))["processed_data"]).float()[:self.train_length, :, 0]
        if data_number != 0:
            self.node_feats = self.node_feats[-data_number*288:,:]
        # CNN for global feature extraction
        ## for the dimension, see https://github.com/zezhishao/STEP/issues/1#issuecomment-1191640023
        orig = self.node_feats.shape[0]
        for _ in range(4):
            orig = orig - 10
            orig = int(orig // 2)
            orig = orig + 1
        ...
        ## network structure
        self.conv1 = torch.nn.Conv1d(1, 8, 10, stride=2)  # .to(device)
        self.conv2 = torch.nn.Conv1d(8, 16, 10, stride=2)  # .to(device)
        self.conv3 = torch.nn.Conv1d(16, 16, 10, stride=2)
        self.conv4 = torch.nn.Conv1d(16, 16, 10, stride=2)
        ...
        ## for the dimension, see https://github.com/zezhishao/STEP/issues/1#issuecomment-1191640023
        self.dim_fc_mean = {"METR-LA": 16128, "PEMSD7M": 16128 * 2, "PEMS-BAY": 16128, 'HKTSM':16128}[dataset_name]
    ...
    def get_k_nn_neighbor(self, data, k=11*207, metric="cosine"):
        ...
        if len(batch_sim.shape) == 3:
            ...
            # no batch dimension
            num_nodes, _ = batch_sim.shape
            adj = batch_sim.view(num_nodes*num_nodes)
            res = torch.zeros_like(adj)
            top_k, indices = torch.topk(adj, k, dim=-1)
            res.scatter_(-1, indices, top_k)
            adj = torch.where(res != 0, 1.0, 0.0).detach().clone()
    ...
    def forward(self, long_term_history, tsformer=None, compute_hidden=True):
        ...
        # print('node_feats', self.node_feats.shape) (23990, 207), full time series
        global_feat = self.node_feats.to(device).transpose(1, 0).view(num_nodes, 1, -1) # (207, 1, 23990)
        # global_feat = self.bn2(F.relu(self.conv2(self.bn1(F.relu(self.conv1(global_feat))))))
        # remove bn
        global_feat = F.relu(self.conv2(F.relu(self.conv1(global_feat))))
        global_feat = F.relu(self.conv4(F.relu(self.conv3(global_feat))))
        
        # print("global_feat", global_feat.shape) 207, 16, 23972
        global_feat = global_feat.view(num_nodes, -1)
        # print("global_feat", global_feat.shape) 207, 383552
        global_feat = F.relu(self.fc(global_feat))

        # print("global_feat", global_feat.shape) 207, 100
        
        
        ## global_feat = self.bn3(global_feat)
        # remove bn
        # print("global_feat before expansion: ", global_feat.shape) # 207, 100
        
        ## try to remove the #batch_size gumbel sampling
        # global_feat = global_feat.unsqueeze(0).expand(batch_size, num_nodes, -1)                     # Gi in Eq. (2)
        # print("global_feat after expansion: ", global_feat.shape) (32, 207, 100)
        # generate dynamic feature based on TSFormer

        ## mask out the transformer inference
        if compute_hidden and tsformer:
            hidden_states = tsformer(long_term_history[..., [0]])
            # print('hidden_states', hidden_states.shape) # (batch_size, num_node, num_token, embed_dim)
        else:
            hidden_states = None

        # print('hidden_states', hidden_states.shape)
        # print('hidden_states', hidden_states.shape)
        # print("hidden_states from tsformer", hidden_states.shape) (8, 207, 168, 96), batch_size, num_node, num_patch, embed_dim
        ...
        ## with batch_size as first dim
        # sampled_adj = sampled_adj[..., 0].clone().reshape(batch_size, num_nodes, -1)

        ## without batch_size as the first dim
        sampled_adj = sampled_adj[..., 0].clone().reshape(num_nodes, -1)
        # print('sampled_adj.shape', sampled_adj.shape)
        ## remove self-loop, with batch_size as first dim
        ...
        # first reshape the num_patch and embed_dim together
        # print('graph connection ratio', self.k)

        ## also mask out the adj_knn
        # adj_knn with batch_size as the first dim
        ...
        # adj_knn without the batch size as the first dim
        adj_knn = self.get_k_nn_neighbor(node_feat.reshape(num_nodes, -1), k=self.k*self.num_nodes, metric="cosine")
        mask = torch.eye(num_nodes, num_nodes).bool().to(adj_knn.device)
        adj_knn.masked_fill_(mask, 0)
